...
def render_haha(rgb_BS3HW, sigma_BS1HW, xyz_BS3HW, use_alpha=False, is_bg_depth_inf=False):
    if not use_alpha:
        imgs_syn, depth_syn, blend_weights, weights = plane_volume_rendering_haha(
            rgb_BS3HW,
            sigma_BS1HW,
            xyz_BS3HW,
            is_bg_depth_inf
...
def plane_volume_rendering_haha(rgb_BS3N, sigma_BS1N, xyz_BS3N, is_bg_depth_inf):
    #print("rgb_BS3HW", rgb_BS3HW.shape)
    #print("sigma_BS1HW", sigma_BS1HW.shape)
    #print("xyz_BS3HW", xyz_BS3HW.shape)
    B, S, _, N = sigma_BS1N.size()

    xyz_diff_BS3N = xyz_BS3N[:, 1:, :, :] - xyz_BS3N[:, 0:-1, :, :]  # Bx(S-1)x3xN
    xyz_dist_BS1N = torch.norm(xyz_diff_BS3N, dim=2, keepdim=True)  # Bx(S-1)x1xN

    xyz_dist_BS1N = torch.cat((xyz_dist_BS1N,
                                torch.full((B, 1, 1, N),
                                           fill_value=1e3,
                                           dtype=xyz_BS3N.dtype,
                                           device=xyz_BS3N.device)),
                               dim=1)  # BxSx1xN
    #print("xyz_dist_BS1N", xyz_dist_BS1N.shape)
    transparency = torch.exp(-sigma_BS1N * xyz_dist_BS1N)  # BxSx1xN
    alpha = 1 - transparency # BxSx1xN

    # add small eps to avoid zero transparency_acc
    # pytorch.cumprod is like: [a, b, c] -> [a, a*b, a*b*c], we need to modify it to [1, a, a*b]
    transparency_acc = torch.cumprod(transparency + 1e-6, dim=1)  # BxSx1xN
    transparency_acc = torch.cat((torch.ones((B, 1, 1, N), dtype=transparency.dtype, device=transparency.device),
                                  transparency_acc[:, 0:-1, :, :]),
                                 dim=1)  # BxSx1xHxW

    weights = transparency_acc * alpha  # BxSx1xHxW
    rgb_out, depth_out = weighted_sum_mpi_haha(rgb_BS3N, xyz_BS3N, weights, is_bg_depth_inf)

    return rgb_out, depth_out, transparency_acc, weights

def weighted_sum_mpi_haha(rgb_BS3N, xyz_BS3N, weights, is_bg_depth_inf):
    weights_sum = torch.sum(weights, dim=1, keepdim=False)  # Bx1xN
    rgb_out = torch.sum(weights * rgb_BS3N, dim=1, keepdim=False)  # Bx3xN

    if is_bg_depth_inf:
        # for dtu dataset, set large depth if weight_sum is small
        depth_out = torch.sum(weights * xyz_BS3N[:, :, 2:, :], dim=1, keepdim=False) \
                    + (1 - weights_sum) * 1000
    else:
        depth_out = torch.sum(weights * xyz_BS3N[:, :, 2:, :], dim=1, keepdim=False) \
...
def plane_volume_rendering(rgb_BS3HW, sigma_BS1HW, xyz_BS3HW, is_bg_depth_inf):
    ...
    xyz_dist_BS1HW = torch.cat((xyz_dist_BS1HW,
                               ...
                               dim=1)  # BxSx1xHxW
    #print("xyz_dist_BS1HW", xyz_dist_BS1HW.shape)
...
def get_src_xyz_from_plane_disparity(meshgrid_src_homo,
    ...
    :return:
    """
    #print("meshgrid_src_homo", meshgrid_src_homo)
    #print("meshgrid_src_homo", meshgrid_src_homo.shape)
