...
class GRUFusion(nn.Module):
    ...
    def convert2dense(self, current_coords, current_values, coords_target_global, tsdf_target, relative_origin,
        ...
        if self.cfg.FUSION.FULL is False:
            ...
            valid_temp = valid.clone()
            valid[valid_temp] = all_true
        # sparse to dense
        ...
        if self.cfg.FUSION.FULL is True:
            ...
            if self.direct_substitude:
                updated_coords = torch.nonzero((global_volume.abs() < 0.3).any(-1) | (current_volume.abs() < 0.3).any(-1))
                mask = torch.ones_like(updated_coords[:, 0]).bool()
        ...
        return updated_coords, current_volume, global_volume, target_volume, valid, valid_target, mask
    ...
    def forward(self, coords, values_in, inputs, scale=2, outputs=None, save_mesh=False):
        ...
        for i in range(batch_size):
            ...
            updated_coords, current_volume, global_volume, target_volume, valid, valid_target, mask = self.convert2dense(
                coords_b,
                values,
                coords_target,
                tsdf_target,
        ...
        if self.direct_substitude:
            ...
            return updated_coords_all, values_all, tsdf_target_all, occ_target_all, mask
