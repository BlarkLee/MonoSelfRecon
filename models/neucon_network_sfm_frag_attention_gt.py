...
class NeuConNet(nn.Module):
    ...
    def forward(self, features, inputs, outputs, extrinsics_last, proj_matrices_last, imgs_last, segments_last):
        ...
        for i in range(self.cfg.N_LAYER):
            ...
            if tsdf_target is not None:
                gt_loss = self.compute_loss(tsdf, occ, tsdf_target, occ_target,
                                         mask=grid_mask,
                                         pos_weight=self.cfg.POS_WEIGHT)
            else:
                gt_loss = torch.Tensor(np.array([0]))[0]
            gt_loss_dict[scale] = gt_loss
            ...
            if True: 
                ...
                for (i,j) in mask_all.keys():
                    ...
                    if do_frag and (i<6 or i>11 or j<6 or j>11 or abs(i-j)>4):
                        continue
        ...
        if True:
            ...
            gt_loss = 0.26*gt_loss_dict[0] + 0.33*gt_loss_dict[1] + 0.41*gt_loss_dict[2]
            total_loss = rgb_loss + 0.05*planar_loss
            total_loss = gt_loss + 0.1*total_loss
        
        src_img_xy = None 
        tgt1_img_xy = None
        src_idx = None
        tgt1_idx = None 
        return outputs, rgb_loss, planar_loss, gt_loss, total_loss, src_depth, tgt1_depth, src_img_xy, tgt1_img_xy, src_idx, tgt1_idx
    ...
    def compute_loss(tsdf, occ, tsdf_target, occ_target, loss_weight=(1, 1),
        ...
        if n_p == 0:
            ...
            return torch.Tensor(np.array([0]))[0] #torch.Tensor([0.0]).cuda()[0] * tsdf.sum()
        ...
        tsdf_target = apply_log_transform(tsdf_target[occ_target]*0.12)
