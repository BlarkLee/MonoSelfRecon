...
def eval_mesh(file_pred, file_trgt, threshold=.05, down_sample=.02):
    ...
    _, dist1 = nn_correspondance(verts_pred, verts_trgt) #comp
    _, dist2 = nn_correspondance(verts_trgt, verts_pred) #acc
...
def eval_depth(depth_pred, depth_trgt):
    ...
    #mask1 = (depth_pred < 3) * (depth_pred > 0)  # ignore values where prediction is 0 (% complete)
