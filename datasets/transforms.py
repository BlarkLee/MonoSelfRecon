...
class IntrinsicsPoseToProjection(object):
    ...
    def __call__(self, data):
        ...
        proj_matrices = []
        pose_aligncam2cam = []
        for j, (intrinsics, extrinsics) in enumerate(zip(data['intrinsics'], data['extrinsics'])):
            pose_aligncam2cam.append(middle_pose @ torch.inverse(extrinsics.data.cpu()))
            ...
            for i in range(3):
                ...
                scale_intrinsics = intrinsics / 2 ** i
        ...
        data['pose_aligncam2cam'] = torch.stack(pose_aligncam2cam)
        return data
...
class RandomTransformSpace(object):
    ...
    def __init__(self, voxel_dim, voxel_size, random_rotation=False, random_translation=False,
    ...
    def transform(self, data, transform=None, old_origin=None,
        ...
        center = (torch.tensor(((bnds[0, 1] + bnds[0, 0]) / 2, (bnds[1, 1] + bnds[1, 0]) / 2, -0.2)) - data[
            'vol_origin']) / self.voxel_size    # Before "-data['vol_origin']" and "/self.voxel_size" is the real world coordinate
        ...
        origin[2] = center[2]
        vol_origin_partial = origin * self.voxel_size + data['vol_origin'] # real world coordinate of the 3D vox origin of a single frame (use self.max_depth to derive)
        ...
        if 'tsdf_list_full' in data.keys():
            ...
            coords = (world - old_origin.T) / self.voxel_size  #the relative vox coordinate of the self-built vox (use max_depth=3 to recover) at the origin of scannet original vox origin label
            ...
            for l, tsdf_s in enumerate(data['tsdf_list_full']):
                ...
                occ_vol[(tsdf_vol < 1) & (tsdf_vol > -1) & (weight_vol > 1)] = True
